...
fn active_no_commit(snap: &Snapshot, alerts: &mut Vec<Alert>) {
    // Host-wide process names cannot distinguish this loop from an editor or
    // another repository. Use only repo-scoped log activity for this tripwire.
    let active = snap.logs.iter().any(|l| l.mtime_age_min <= FRESH_LOG_MIN);
    if !active {
        return;
    }
...
fn claimable_all_idle(snap: &Snapshot, alerts: &mut Vec<Alert>) {
    ...
    if no_leases && no_fresh_log {
        alerts.push(Alert {
            severity: Severity::Stall,
            message: format!(
                "claimable={} at {} but all factory lanes idle (no lease, no fresh loop log)",
...
mod tests {
    ...
    fn active_no_commit_stalls() {
        ...
        snap.logs.push(LogView {
            ...
            mtime_rel: "1m".into(),
            mtime_age_min: 1,
            signals: vec!["iteration active".into()],
        });
        let alerts = evaluate(&snap);
        assert!(
            alerts
    ...
    #[test]
    fn unrelated_host_provider_does_not_make_the_repo_active() {
        let mut snap = base_snap();
        snap.head_age_min = Some(90);
        snap.git.head_age_min = Some(90);
        snap.procs.claude = 1;
        ...
        assert!(
            alerts.iter().all(|a| !a.message.contains("HEAD age")),
            "{alerts:?}"
        );
    }
