//! Contract snapshots: the generated `(australia_register, interest)` and
...
//! `UPDATE_SNAPSHOT=1 cargo test -p australia_register --test details_schema_snapshot`
//! and commit the diff.
    ...
    let committed = fs::read_to_string(&path).unwrap_or_else(|_| {
        panic!(
            "missing committed snapshot at {}; generate it with \
             `UPDATE_SNAPSHOT=1 cargo test -p australia_register --test details_schema_snapshot` and commit it",
            path.display()
        )
    });
