//! Contract snapshot: the generated `(us_senate, transaction)` details schema
//! must match the committed copy the conformance registry embeds
//! (`crates/pipeline/schemas/details/us_senate.transaction.json`) — invariant 5.
//! Regenerate deliberately with
//! `UPDATE_SNAPSHOT=1 cargo test -p us_senate --test details_schema_snapshot`
//! and commit the diff.
...
fn details_schema_matches_committed_snapshot() {
    let schema = us_senate::details::transaction_details_schema();
    ...
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("schemas")
        .join("details")
        .join("us_senate.transaction.json");
    ...
    let committed = fs::read_to_string(&path).unwrap_or_else(|_| {
        panic!(
            "missing committed snapshot at {}; generate it with \
             `UPDATE_SNAPSHOT=1 cargo test -p us_senate --test details_schema_snapshot` and commit it",
            path.display()
        )
    });
    ...
    assert_eq!(
        committed, current,
        "(us_senate, transaction) details schema drifted from the committed snapshot; if the \
