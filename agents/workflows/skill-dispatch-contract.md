...
Runtime ownership is separate from dispatch policy: the pre-built Rust supervisor is
the only component allowed to invoke Codex or Claude, own provider process trees, retry,
or fail over. `agents/run-loop-codex.sh` is a validation compatibility shim that delegates
to that supervisor; it must never grow a raw provider loop.
