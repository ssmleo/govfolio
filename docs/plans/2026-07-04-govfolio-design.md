...
| D10 | Skill governance + external libraries *(founder-approved amendments A1–A3)* | **Standing/situational skills + packs (A1); doer split into rust-builder/web-builder along the D7 boundary (A2); bespoke>imported precedence with vendored-pin imports only (A3). superpowers@d884ae04 vendored+screened and ACTIVE; impeccable + rust/ts packs ARMED via goal 019.** | Ceiling caps per-iteration context breadth, so trigger-loaded situational skills and coherent packs don't count against it. The builder split is the language boundary reaching the role layer — forced honestly by the TS skill list rather than by a ceiling exception. Live marketplace installs forbidden because an auto-update is an unreviewed prompt change; every import enters pinned, screened, line-audited, and founder-sampled. extraction-strategy becomes spec-writer-exclusive: strategy is decided once, recorded in the SAF, and builders implement from the record. |
| D11 | Effort & ultracode dispatch | **Per-role effort bound through thin native `.claude/agents` shims (policy in `agents/EFFORT.md`); ultracode invoked per-task by the orchestrator on an eligible-class registry, never session-wide.** | Frontmatter effort is only honored on native subagents, so shims bridge our governed roles to Claude Code's harness without duplicating behavior (role file stays the single source; shim loses on conflict). Per-task keyword keeps the docs' cost guardrail; first-of-class runs reduced scope. Epistemics unchanged: workflow self-verification never replaces conformance/validators/auditor — world-verifies-model remains the done-signal. |
...
6. **Governance, skills, orchestration:** roles compose from a shared skill catalog (`agents/skills/*/SKILL.md` — one procedure per skill, deepened by write-back, never forked per role). **Founder gate:** no role gains or changes a `Skills:` section without recorded founder approval (`agents/GOVERNANCE.md`, applied retroactively to the initial nine). A dedicated **orchestrator** role runs the deterministic per-iteration workflow (`agents/workflows/orchestration.md`): triage human gates → select by strict priority (CI red → drift → queue → factory) → gate-check → dispatch role+skills+SAF → verify (never self-certifies) → record → journal. It never writes production code, approves proposals, or unblocks human lanes.
